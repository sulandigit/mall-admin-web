  ...
  <div class="menu-wrapper">
    <template v-for="item in routes" v-if="!item.hidden&&item.children">
      ...
      <router-link 
        ...
        @mouseenter.native="handleMenuHover(item.children[0].name)"
      >
        <el-menu-item 
      ...
      <el-submenu 
        ...
        :key="item.name"
        @mouseenter.native="handleMenuHover(item.name)"
      >
        <template slot="title">
        ...
        <template v-for="child in item.children" v-if="!child.hidden">
          ...
          <router-link 
            ...
            :key="child.name"
            @mouseenter.native="handleMenuHover(child.name)"
          >
            <el-menu-item :index="item.path+'/'+child.path">
              <svg-icon v-if="child.meta&&child.meta.icon" :icon-class="child.meta.icon"></svg-icon>
              <span v-if="child.meta&&child.meta.title" slot="title">{{child.meta.title}}</span>
...
export default {
  ...
  methods: {
    ...
    handleMenuHover(routeName) {
      ...
      // 记录悬停行为并触发预加载
      permissionPreloader.handleMenuHover(routeName)
    }
  }
}
</script>
